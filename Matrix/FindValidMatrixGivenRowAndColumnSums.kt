/*
* Find Valid Matrix Given Row and Column Sums
You are given two arrays rowSum and colSum of non-negative integers where rowSum[i] is the sum of the elements in the ith row and colSum[j] is the sum of the elements of the jth column of a 2D matrix.
In other words, you do not know the elements of the matrix, but you do know the sums of each row and column.
Find any matrix of non-negative integers of size rowSum.length x colSum.length that satisfies the rowSum and colSum requirements.
Return a 2D array representing any matrix that fulfills the requirements.
It's guaranteed that at least one matrix that fulfills the requirements exists.

Example 1:
Input: rowSum = [3,8], colSum = [4,7]
Output: [[3,0],
         [1,7]]
Explanation:
0th row: 3 + 0 = 3 == rowSum[0]
1st row: 1 + 7 = 8 == rowSum[1]
0th column: 3 + 1 = 4 == colSum[0]
1st column: 0 + 7 = 7 == colSum[1]
The row and column sums match, and all matrix elements are non-negative.
Another possible matrix is: [[1,2],
                             [3,5]]

Example 2:
Input: rowSum = [5,7,10], colSum = [8,6,8]
Output: [[0,5,0],
         [6,1,0],
         [2,0,8]]

Constraints:
1 <= rowSum.length, colSum.length <= 500
0 <= rowSum[i], colSum[i] <= 108
sum(rowSum) == sum(colSum)
*/

fun main() {
    val result = restoreMatrix(intArrayOf(3, 8), intArrayOf(4, 7))
    result.forEach { row ->
        println(row.joinToString(" "))
    }
}

fun restoreMatrix(rowSums: IntArray, colSums: IntArray): Array<IntArray> {
    val result = Array(rowSums.size) { IntArray(colSums.size) }

    for (rowIndex in rowSums.indices) {
        for (colIndex in colSums.indices) {
            val value = minOf(rowSums[rowIndex], colSums[colIndex])

            result[rowIndex][colIndex] = value
            rowSums[rowIndex] -= value
            colSums[colIndex] -= value
        }
    }

    return result
}

class Solution {
    fun restoreMatrix(rowSums: IntArray, colSums: IntArray): Array<IntArray> {

        val result = Array(rowSums.size) { y ->
            makeRow(rowSums[y], colSums)
        }

        return result
    }

    fun makeRow(rowSum: Int, colSums: IntArray): IntArray {
        var rowSum = rowSum
        val row = IntArray(colSums.size)

        for (x in colSums.indices) {
            val value = minOf(rowSum, colSums[x])
            colSums[x] -= value
            rowSum -= value
            row[x] = value
        }

        return row
    }
}

class Solution {
    fun restoreMatrix(rowSums: IntArray, colSums: IntArray): Array<IntArray> {
        val newColSums = Array(colSums.size) { Counter(colSums[it]) }
        val newRowSums = Array(rowSums.size) { Counter(rowSums[it]) }
        val result = Array(rowSums.size) { y ->
            buildLine(newRowSums[y], newColSums)
        }

        return result
    }

    private fun buildLine(sum: Counter, colSums: Array<Counter>): IntArray {
        val row = IntArray(colSums.size)
        colSums.forEachIndexed { x, counter ->
            row[x] = sum.min(counter)
        }

        return row
    }

    class Counter(var count: Int) {

        fun min(counter: Counter): Int {
            val value = minOf(count, counter.count)
            count -= value
            counter.count -= value
            return value
        }
    }

}

class Solution {
    fun restoreMatrix(rowSum: IntArray, colSum: IntArray): Array<IntArray> {
        val m: Int = rowSum.size
        val n: Int = colSum.size
        val sol: Array<IntArray> = Array(m) { IntArray(n) { 0 } }
        var r = 0
        var c = 0

        while (r < m && c < n) {
            val tmp = minOf(rowSum[r], colSum[c])

            sol[r][c] = tmp
            rowSum[r] -= tmp
            colSum[c] -= tmp

            if (rowSum[r] > 0) {
                c += 1
                continue
            }

            if (colSum[c] > 0) {
                r += 1
                continue
            }

            c += 1
            r += 1
        }

        return sol
    }
}