...
{
public:
    void dfs(vector<vector<int>> &mat, int i, int j, int val, int newColor)
    {
        if (i < 0 || i >= mat.size() || j < 0 || j >= mat[0].size() || mat[i][j] == newColor || mat[i][j] != val)
            return;

        mat[i][j] = newColor;
        dfs(mat, i - 1, j, val, newColor);
        dfs(mat, i + 1, j, val, newColor);
        dfs(mat, i, j - 1, val, newColor);
        dfs(mat, i, j + 1, val, newColor);
    }
